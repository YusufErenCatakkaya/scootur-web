<template>
  <div class="max-w-4xl mx-auto py-8 px-4">
    <div class="mb-8">
      ...
      <h1 class="text-3xl font-bold text-gray-900">Kaza Tutanağını Düzenle</h1>
    ...
    <div class="glass-card p-8 animate-fade-in-up">
      <form @submit.prevent="submitReport" class="space-y-8">
        ...
        <!-- Kaza Özeti ve Çizim -->
        <div>
          <h3 class="text-xl font-semibold border-b pb-2 mb-4 text-gray-800">5. Kazanın Oluş Şekli ve Taslağı</h3>
          <div class="space-y-6">
...
onMounted(async () => {
  ...
  try {
    ...
    if (report) {
      form.value.date = report.date
      form.value.time = report.time
      form.value.province = report.province
      onProvinceChange()
      form.value.district = report.district
      const addressParts = report.address.split(', ')
      form.value.neighborhood = addressParts[2] || ''
      
      const parsedFormData = JSON.parse(report.formData || '{}')
      form.value.formData = { ...form.value.formData, ...parsedFormData }
      
      // Handle photos
      if (parsedFormData.photos && Array.isArray(parsedFormData.photos)) {
        photos.value = parsedFormData.photos.map(url => ({ preview: url }))
      }
      if (parsedFormData.invoiceImage) {
        invoicePreview.value = parsedFormData.invoiceImage
      }
      
      // Add existing images to base64 arrays so we can re-upload them if needed? 
      // No, we shouldn't re-upload if they already exist, we keep them in formData.photos / formData.invoiceImage
    }
  } catch(err) {
    console.error('Veri yüklenemedi', err)
    alert('Tutanak bilgileri yüklenemedi.')
    router.push('/reports')
  } finally {
    loading.value = false
  }
